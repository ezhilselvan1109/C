...
int leapyear(int);
int Duedays(int,int,int,int,int,int);
...
void BooksReturn(){
    ...
    while(fread(&student[i],sizeof(struct Library),1,file)){
        if(student[i].Stud_Regno==Regno){
            printf("|%-12s |",student[i].Stud_name);
            printf("%-11s |",student[i].Stud_Dept);
            printf("%-9d |",student[i].Books_ID);
            printf("%-14s|",student[i].Name_Of_books);
            ...
            scanf("%d",&bookid);
                while(fread(&student[i],sizeof(struct Library),1,file)){
                    if(student[i].Stud_Regno==Regno){
                            ...
                            printf("%-11s |",student[i].Stud_Dept);
                            printf("%-9d |",student[i].Books_ID);
                            printf("%-13s |",student[i].Name_Of_books);
                            printf(" %d/%d/%-6d |\n",student[i].Issued_date,student[i].Issued_month,student[i].Issued_year);
                            printf("+-------------+------------+----------+--------------+-------------+\n\n\n");
                            ...
                            int days=Duedays(returning_date,returning_month,returning_year,Issued_date,Issued_month,Issued_year);
                            student[i].due_amount=Calculate_Amount(days);
                            printf("\n\n\t\t\tBOOK TAKEN %d DAYS BEFORE \n",days);
...
int Duedays(int returning_date,int returning_month,int returning_year,int Issued_date,int Issued_month,int Issued_year){
...
void IssuseDetails(){
    ...
    while(fread(&student[i],sizeof(struct Library),1,file)){
            printf("| %-12d |",student[i].Stud_Regno);
            printf(" %-12s |",student[i].Stud_name);
            printf(" %-10s |",student[i].Stud_Dept);
            printf(" %-8d |",student[i].Books_ID);
            printf("%-17s |",student[i].Name_Of_books);
...
void ReturnsDetails(){
    ...
    while(fread(&student[i],sizeof(struct Library),1,returnedfile)){
            printf("| %-13d |",student[i].Stud_Regno);
            printf(" %-12s |",student[i].Stud_name);
            printf("%-11s |",student[i].Stud_Dept);
            printf("%-9d |",student[i].Books_ID);
            printf("%-17s |",student[i].Name_Of_books);

            printf("%d/%d/%-7d |",student[i].Issued_date,student[i].Issued_month,student[i].Issued_year);
